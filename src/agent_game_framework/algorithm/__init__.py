...
``AlgorithmRecommendation``, and ``AdvisedLLMSeatController``.
...
``AdvisedLLMSeatController`` (``advised_llm.py``) is different: per
``docs/PLAN.md``'s package layout note, "``agent_game_framework.algorithm``
hosts the ``GameAlgorithm`` Protocol and both composite
``SeatController``s ... these are framework code, reused unchanged by every
game" -- it legitimately depends on ``agent_game_framework.core`` to compose
