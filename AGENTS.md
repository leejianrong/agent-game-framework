...
banter), V4 (standalone game algorithm, two composition modes), and V5
(`Conversable` chat, decoupled from moves) complete.** `docs/PLAN.md`,
`docs/SLICES.md`, and `docs/adr/*` are the source of truth for *what* is
being built and *why*. This file is the source of truth for *how* to work in
this repo day to day. If this file, the docs, and the actual code ever
disagree, trust the code — then fix whichever doc is stale.

## What this is

An installable Python framework for playing turn-based/simultaneous-move games
(Tic-Tac-Toe now; Catan, Poker, Wavelength, Codenames later, each in its own
sibling repo) with any mix of human and AI-controlled seats, including
zero-human matches. See `docs/PLAN.md` for the full problem/solution and
`docs/adr/0001` through `0009` for the architectural decisions already locked
in.
...
- V5 (`Conversable` chat capability) is done: the `Conversable` Protocol +
  `TextTurn`/`ConversationTurn` (`core/conversable.py`, ADR-0008, text-only
  for now), `OpenRouterBackend.respond()` (a second, unconstrained
  chat-completions call, separate from `decide()`'s constrained
  `submit_move` tool call), pass-through `respond()` on both composite seat
  controllers so chat "sees through" `advised-by=`/`narrated-by=`
  composition exactly like `decide()` already does, `LiveMatch`
  (`core/live_match.py`, ADR-0009 — runs a `Match` on a background thread so
  chat works even while a seat's own `decide()` call is in flight), and the
  CLI's chat-mode wiring in `cmd_play`/`run_live_match_with_chat` (a human
  seat can freely chat with a `Conversable` opponent any time it isn't their
  own turn; on their own turn, a `"chat: <message>"` prefix disambiguates a
  chat line from a move attempt). The framework's own `cli.py` has zero
  Tic-Tac-Toe-specific code as of this slice — board rendering moved to
  `examples/tictactoe/cli_render.py`, looked up generically via
  `RENDERER_REGISTRY` (optional per game), the same pattern
  `GAME_REGISTRY`/`ALGORITHM_REGISTRY` already used.
...
make play              # play tic-tac-toe: you vs. a random bot
make demo              # play vs. unbeatable minimax, narrated + chattable via an LLM -- needs OPENROUTER_API_KEY in .env
make demo-advised      # play vs. an LLM that decides for itself, advised by minimax (can still lose)
make lint              # ruff check .
make typecheck         # mypy src examples
...
- **`OpenRouterBackend` (V3) and any live-API test must mock the HTTP call.**
  ...
  of `make test` or CI.
- **`LiveMatch` (V5) tests synchronize with `threading.Event`, never
  `time.sleep`-based races.** A fake controller signals "my `decide()` call
  has genuinely started" via one event and blocks on a second until the test
  releases it — see `tests/integration/test_live_match.py` — so "chat while
  a seat's own move-decision is in flight" is provably true every run, not
  just likely under favorable scheduling.
